...
(function () {
  ...
  function loadBase(s) {
    ...
    img.onload = () => { s.baseTex = upload(img); schedule(); };
  ...
  function loadDetail(s, li, volatile) {
    ...
    img.onload = () => {
      if (cancelled) return;
      // Still worth having by the time it arrived?
      if (volatile && (!onScreen(s) || wantedLevel(s) !== li)) {
        done();
  ...
  function refine() {
    ...
    for (const s of shots) {
      ...
      if (p && p.volatile && (!onScreen(s) || wantedLevel(s) !== p.level)) {
        p.cancel();
      }
    }
    ...
    visible
      ...
      .slice(0, Math.max(0, MAX_INFLIGHT - inflight))
      .forEach((s) => loadDetail(s, wantedLevel(s), true));
    updateHud();
  }
  ...
  function footprintPass(VP, vis, W, H, feather, outFB, gain) {
    ...
    for (const i of vis) {
      ...
      gl.uniformMatrix3fv(uSpin, false, lifting ? introSpin(s) : IDENT3);
  ...
  // ── Opening flight ────────────────────────────────────────────────
  /* The photographs arrive rather than being already there. Each starts
   * somewhere else entirely on the sky — a point drawn at random from the
   * whole sphere — turns as it comes, and settles onto its own patch.
   *
   * It is a rotation from the random start to the real position, about the
   * axis perpendicular to the two, which is the great circle between them:
   * the short way round. That keeps every frame ON the sphere for the whole
   * journey, at its true size, so nothing swells or shrinks as it travels
   * and nothing has to be un-projected at the end. At rest the rotation is
   * exactly the identity, which is the only way the map is exactly right
   * once the flight is over.
   *
   * A second rotation rides along: a roll about the frame's own centre —
   * an axis running from the origin through the frame — so it spins in its
   * own plane as it travels rather than being carried round flat.
   *
   * Three seconds, and over by then. Each frame gets its own duration
   * inside that, so they land in a scatter rather than in lockstep, but the
   * longest is the full three: an animation that says three seconds and
   * takes four is a page that feels slow for reasons nobody can name.
   *
   * Both parts decay as (1-p)³: quick at first, slow into the last of it,
   * which is the part that reads as settling rather than stopping.
   */
  const INTRO_MS = 3000;         // the whole thing is over by here
  const INTRO_MIN_MS = 2100;     // …and the quickest frame lands here
  const INTRO_SPIN = 2.2;        // radians of roll to unwind, at most
  let introStart = 0;
  let introEnd = 0;              // when the LAST frame lands
  let introRaf = 0;
  ...
  function introStop() {
    // Clearing introEnd is what actually ends the flight — the step
    // function checks it and stops scheduling — so cancelling here only
    // saves the one frame already in flight.
    if (introRaf) cancelAnimationFrame(introRaf);
    introRaf = 0;
    introEnd = 0;
  }

  /* A direction drawn uniformly over the sphere. The cosine of the polar
   * angle is sampled flat, NOT the angle itself, which would bunch every
   * starting point around the poles. */
  function randomSky() {
  ...
  function introBegin() {
    ...
    // Motion turned down: the map is simply there, which is also the
    // fastest way to read it.
    if (typeof window !== "undefined" && window.matchMedia
        && window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    introStart = performance.now();
    for (const s of shots) {
      s.introDur = INTRO_MIN_MS + Math.random() * (INTRO_MS - INTRO_MIN_MS);
      s.introRot = (Math.random() * 2 - 1) * INTRO_SPIN;
      const q = randomSky();
      const c = s.centre;
      // Axis and angle carrying the frame's real direction to the random
      // one: rotate by the full angle at the start of the flight and by
      // nothing at the end.
      let kx = c[1] * q[2] - c[2] * q[1];
      let ky = c[2] * q[0] - c[0] * q[2];
      let kz = c[0] * q[1] - c[1] * q[0];
      let kl = Math.hypot(kx, ky, kz);
      if (kl < 1e-9) {
        // Drawn on top of its own position, or exactly opposite it. Either
        // way the cross product says nothing about which way to go, so
        // take any axis perpendicular to the frame and let the angle
        // decide — zero, or the half turn, both of which are correct.
      ...
      s.introAxis = [kx / kl, ky / kl, kz / kl];
      s.introAng = Math.acos(Math.max(-1, Math.min(1,
        c[0] * q[0] + c[1] * q[1] + c[2] * q[2])));
    }
    introEnd = introStart + INTRO_MS;
    const step = () => {
      introRaf = 0;
      draw();
      // One last frame after the deadline, so the final state drawn is
      // every frame exactly home rather than a fraction short of it.
      if (performance.now() < introEnd) introRaf = requestAnimationFrame(step);
      else { introEnd = 0; draw(); }
    };
  ...
  /* How much of this frame's flight is left, 1 → 0. Zero once it has
   * landed, and zero for every frame when nothing is flying — which is
   * what lets the draw path ask unconditionally. */
  function introEase(s) {
    if (!introEnd || !s.introDur) return 0;
    const p = (performance.now() - introStart) / s.introDur;
    if (p >= 1) return 0;
    const k = 1 - Math.max(0, p);
    return k * k * k;
  }
   ...
   * has landed. */
  function introSpin(s) {
    const k = introEase(s);
    if (!k) return IDENT3;
    const travel = s.introAng ? s.introAng * k : 0;
    const roll = s.introRot ? s.introRot * k : 0;
    if (!travel && !roll) return IDENT3;
    if (!travel) return rotAbout(s.centre, roll, introSpinBuf);
    const T = rotAbout(s.introAxis, travel, introTravelBuf);
    if (!roll) return T;
  ...
  function draw() {
    ...
    /* onScreen() answers "is this frame's REAL position in view", which is
     * the same question as "will it be drawn" only when nothing has been
     * moved. While the edges are live it is not: a frame far off the
     * screen is gathered to just beyond the border and is therefore
     * visible, and culling on the old test would leave holes in that queue
     * which fill in as you pan. Same during the opening flight.
     *
     * So the cull comes back exactly when the effect is off — which is
     * zoomed in, where it is worth the most: one or two frames instead of
     * a hundred and thirty-nine, at the zoom somebody is actually studying
     * a photograph at.
     *
     * Drawing them all costs less than it looks: every frame already holds
     * a tier-0 texture from the moment the dome opens, the gathered ones
     * draw at their true angular size (a few pixels, out there), and
     * detail loading is a separate path that still asks about real
     * positions — so nothing pulls a 2048px tier for a frame parked in the
     * margin.
     */
    const drawAll = introActive();
    const vis = [];
    const bgVis = [];
    for (let i = 0; i < shots.length; i++) {
      if (!(drawAll || onScreen(shots[i]))) continue;
      (shots[i].bg ? bgVis : vis).push(i);
    }
    ...
    openingFlight: () => ({
      ...
      shots: shots.map((s) => {
        ...
        return {
          ...
          travelDeg: (s.introAng || 0) / DEG,
          // The axis the whole journey turns about. With it a caller can
          // reconstruct where the frame set off from and check that it is
          // traveling along the great circle between there and home,
          // rather than having to catch it at a known instant.
          axis: (s.introAxis || [0, 0, 1]).slice(),
          durMs: s.introDur || 0,
        };
      }),
    }),
    beginOpeningFlight: () => { introBegin(); return introActive(); },
