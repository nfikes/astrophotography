...
const NEW_OBJECT_IDS = [
  // New arrivals 2026-09-07 .. 2026-09-13 (the calendar week, Mon-Sun). Newest first.
  // Generated: tools/deposit_inventory.py --new-band
  // 2026-09-10
  "NGC6604",    // 3 image(s) in the album
  // 2026-09-09
  "NGC6960",    // 4 image(s) in the album
  "NGC6605",    // 3 image(s) in the album
];
...
function renderShowcaseScrub(subj, steps) {
  ...
  if (ticks && ticks.dataset.n !== String(steps.length)) {
    ...
    ticks.innerHTML = steps.map((s, i) => {
      ...
      return `<span class="sc-scrub-tick" style="left:${pct.toFixed(3)}%">` +
             `<b>${escapeHtml(fmtMinutes(s.requested_min))}</b></span>`;
    }).join("");
  }
  ...
  if (read && cur) {
    if (frac > 0.001) {
      ...
      read.innerHTML =
        `<strong>${escapeHtml(fmtMinutes(steps[i0].requested_min))}</strong> ` +
        `→ <strong>${escapeHtml(fmtMinutes(steps[i1].requested_min))}</strong> · ` +
        `${Math.round(frac * 100)}%`;
    } else {
      ...
      // minutes delivered.
      read.innerHTML =
        `<strong>${escapeHtml(fmtMinutes(cur.requested_min))}</strong> · ` +
...
function paintSlide() {
  ...
  if (state.mode === "showcase" && subj.showcase) {
    ...
    img.alt = `${item.objName || item.objId} at ${fmtMinutes(cur.requested_min)}`;
    const blend = $("#slide-sc-blend");
    if (blend) {
      if (frac > 0.001) {
    ...
    setDownloadButtons([{ path: cur.path, kind: "integration",
                          note: fmtMinutes(cur.requested_min) }]);
    sizeStage();
...
function slideshowRunning() { return slideshowTimer !== null; }
...
function slideshowArm() {
  ...
  slideshowTimer = setTimeout(() => {
    ...
    // back; wait rather than advance, and pick up when they return.
...
function renderShowcasePanel() {
  ...
  const chips = steps.map((s, i) => `
    <button type="button" class="sc-chip${i === _scIndex ? " is-on" : ""}"
      ...
      ${escapeHtml(fmtMinutes(s.requested_min))}
    </button>`).join("");
  ...
  panel.innerHTML = `
    ...
    <img class="sc-img" src="${escapeAttr(vURL(cur.path))}"
         alt="${escapeAttr(_scItem.objId + " at " + fmtMinutes(cur.requested_min))}" />
